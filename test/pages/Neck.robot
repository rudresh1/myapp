...
Fill Findings 
    ...
    Log   =========== Findings ===========    console=yes
      &{oldFieldListInNeck}    Create Dictionary    Thyroid=Thyroid     Lymph nodes=Lymphnodes    Other findings=Other findings    Rt. Parotid Gland=RightParotidGland        Lt. Parotid Gland=LeftParotidGland        Rt. Submandibular G=RightSubmandibular        Lt. Submandibular G=LeftSubmandibular    
      
    Run keyword if  'templateType' in @{findMember}
    ...    Fill Template Data In Neck    ${findMember}[templateType]    findings
...
Fill Conclusion
    ...
    ...    Fill Template Data In Neck   ${conMember}[templateType]    conclusion
...
Rt. Submandibular G
    ...
    Scroll Element Into View    //table//td[text()='Rt. Submandibular ']
    Select Value For Field    Rt. Submandibular G    ${data}[option]
    Run keyword if  'checkboxOptions' in @{data}    Fill Comment Section    Rt. Submandibular G    ${data}[checkboxOptions]
    
    Run keyword if  'size' in @{data}    Enter Size In Sub-Field    Rt. Submandibular G    ${data}[size]
    Run keyword if  'nm' in @{data}    Toggle Field    Rt. Submandibular G    NM    ${data}[nm]
    
Lt. Submandibular G
    [Documentation]    selecting the options of Right parotid gland
    [Arguments]    ${data}=${EMPTY}
    sleep    2
    Scroll Element Into View    //table//td[text()='Lt. Submandibular ']
    sleep    1
    Select Value For Field    Lt. Submandibular G    ${data}[option]
    sleep    2
    Run keyword if  'checkboxOptions' in @{data}    Fill Comment Section    Lt. Submandibular G    ${data}[checkboxOptions]
    
    Run keyword if  'size' in @{data}    Enter Size In Sub-Field    Lt. Submandibular G    ${data}[size]
    Run keyword if  'nm' in @{data}    Toggle Field    Lt. Submandibular G    NM    ${data}[nm]


Thyroid
    [Documentation]    selecting the options of Right parotid gland
    [Arguments]    ${data}=${EMPTY}
    Scroll Element Into View    //table//td[text()='Thyroid']
    sleep    1
    Select Value For Field    Thyroid    ${data}[option]
    sleep    1
    Run keyword if  'checkboxOptions' in @{data}    Fill Comment Section    Thyroid    ${data}[checkboxOptions]
    
    Run keyword if  'size' in @{data}    Enter Size In Sub-Field    Thyroid    ${data}[size]
    Run keyword if  'nm' in @{data}    Toggle Field    Thyroid    NM    ${data}[nm]
...
Lt. Parotid Gland
    [Documentation]    selecting the options of Right parotid gland
    [Arguments]    ${data}=${EMPTY}
    Scroll Element Into View    //table//td[text()='Lt. Parotid Gland']
    Select Value For Field    Lt. Parotid Gland    ${data}[option]
    Run keyword if  'checkboxOptions' in @{data}    Fill Comment Section    Lt. Parotid Gland    ${data}[checkboxOptions]
    
    Run keyword if  'size' in @{data}    Enter Size In Sub-Field    Lt. Parotid Gland    ${data}[size]
    Run keyword if  'nm' in @{data}    Toggle Field    Lt. Parotid Gland    NM    ${data}[nm]

Rt. Parotid Gland
    [Documentation]    selecting the options of Right parotid gland
    [Arguments]    ${data}=${EMPTY}
    Scroll Element Into View    //table//td[text()='Rt. Parotid Gland']
    Select Value For Field    Rt. Parotid Gland    ${data}[option]
    Run keyword if  'checkboxOptions' in @{data}    Fill Comment Section    Rt. Parotid Gland    ${data}[checkboxOptions]
    
    Run keyword if  'size' in @{data}    Enter Size In Sub-Field    Rt. Parotid Gland    ${data}[size]
    Run keyword if  'nm' in @{data}    Toggle Field    Rt. Parotid Gland    NM    ${data}[nm]
